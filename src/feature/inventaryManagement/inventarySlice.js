...
export const createProductHandlerAsync = createAsyncThunk(
  ...
  async ({ token, data }, { rejectWithValue }) => {
    try {
      ...
      // FIX: use rejectWithValue so Redux gets a clean serializable error
      const errors = error?.response?.data?.errors || ["Something went wrong. Please try again."];
...
export const inventarySlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getInventaryDashbordAsync.fulfilled, (state, action) => {
      ...
      // ✅ If your dashboard API returns current metal rates, map them here:
      state.goldRate = action.payload?.goldRate || state.goldRate;
      state.silverRate = action.payload?.silverRate || state.silverRate;
    });
    ...
    builder.addCase(
      vendorPerformanceDetailsAnalysis.fulfilled,
      (state, action) => {
        state.isLoading = false;
    ...
    builder.addCase(
      vendorPerformanceDetailsAnalysis.rejected,
      (state, action) => {
        state.isLoading = false;
        state.error = action;
    ...
    builder.addCase(updateMetalPriceAsync.fulfilled, (state, action) => {
      state.isLoading = false;
      // ✅ Save updated rate to Redux so CreateNewProduct can read it
      if (action.meta.arg.data.category === "gold") {
        state.goldRate = Number(action.meta.arg.data.price);
      }
      if (action.meta.arg.data.category === "silver") {
        state.silverRate = Number(action.meta.arg.data.price);
      }
    });
